...
Sumdata1 <- data1 %>%
  subset(Target == 2 & Year >2009 & Depth <101 |
           Target == 117 & Month == "July" & Year >2009 |
           Target == 118 & Month == "July" & Year >2009) 
...
Sumdata2 <-aggregate(Sumdata1$Temperature, by=list(Year=Sumdata1$Year, Target=Sumdata1$Target, Depth=Sumdata1$Depth), FUN=mean)%>%
  renameCol('x','mean_T')
...
Sumdata2<-subset(Sumdata2, Year>=(max(Year)-10))

Sumdata2<-subset(Sumdata2, Target==2)


Sumdata3 <-aggregate(Sumdata1$Temperature, by=list(Target=Sumdata1$Target, Depth=Sumdata1$Depth), FUN=mean)%>%
  renameCol('x','mean_T')
...
ggplot(Sumdata2, aes(x=mean_T, y=Depth, color =as.factor(Year))) +
  geom_line(size=2) +
##  scale_color_manual(values=c('mediumaquamarine','sienna1','mediumpurple1','orchid2','darkseagreen4','black','black'), name='Year')+
##  geom_line(data=Sumdata3, aes(color=Common), size = 1.5) +
  scale_y_reverse() +
  scale_x_continuous(limits=c(0, NA), breaks=seq(22), expand=c(0,0)) +
  labs(title = "Lake Superior Water Temperatures",
       subtitle = "USGS nearshore bottom trawl assessments",
       caption = ann_data_access,
